...
const GalleryProjectDetail = ({ project, images }: GalleryProjectDetailProps) => {
  return (
      ...
      <section className="pt-8 pb-8 px-8 md:px-16 lg:px-24">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-8">
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                {project.tags.map((tag, index) => (
                  <span
                    key={index}
                    className="px-3 py-1 text-sm border border-foreground/20 rounded-full text-foreground/70"
                  >
                    {tag}
                  </span>
                ))}
            ...
            {/* Right: Description */}
            <motion.p
              className="text-base text-foreground/70 leading-relaxed max-w-md md:text-right"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
